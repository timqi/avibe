...
class ReplyEnhancerPlatformTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_prompt_includes_harness_architecture_and_memory_context(self):
        ...
        self.assertIn("Use `vibe agent run --async ...` when one Agent delegates work", prompt)
        self.assertIn("Avibe injects this Session as the default async callback target", prompt)
        self.assertIn("Pass `--no-callback` only when you intentionally want no automatic follow-up", prompt)
